...
public:
    ...
    void StopFry() {
        ...
        // Освобождаем горелку
        std::osyncstream(std::cout) << "Frying stopped: sausage #" << this->GetId()
                                        << std::endl;
...
public:
    ...
    void StartBake(GasCooker& cooker, Handler handler) {
        ...
        cooker.UseBurner([self = shared_from_this(), handler = std::move(handler)] {
            ...
            std::osyncstream(std::cout) << "Baking started: bread #" << self->GetId()
                                        << std::endl;
            handler();
        });
    }
    ...
    void StopBaking() {
        ...
        std::osyncstream(std::cout) << "Baking stopped: bread #" << this->GetId()
                                        << std::endl;
        // Освобождаем горелку
        gas_cooker_lock_.Unlock();
    }
