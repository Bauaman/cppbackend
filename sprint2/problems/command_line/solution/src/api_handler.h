...
// Methods, no authorization required ->
    ...
    http::response<http::string_body> HandleMapRequest() {
        ...
        if (r_data_.r_target == "maps") {
            ...
            /*const model::Map**/std::optional<std::shared_ptr<model::Map>> map = gs_.FindMap(id_);
            if (map != std::nullopt) {
                std::vector<std::string> keys_in_map = map->get()->GetKeys();
                for (const auto& key : keys_in_map) {
                    if (key == "id") {
                        resp_message["id"] = *map->get()->GetId();
                    //} else if (key == "dogSpeed") {
                    //    resp_message["dogSpeed"] = map->GetMapDogSpeed();
                    } else if (key == "name") {
                        resp_message["name"] = map->get()->GetName();
                    } else if (key == "roads") {
...
// Methods, authorization required ->
    ...
    http::response<http::string_body> HandlePlayersListRequest() {
        ...
        return ExecuteAuthorized([this](std::shared_ptr<const model::Player> player) {
            boost::json::object resp;
